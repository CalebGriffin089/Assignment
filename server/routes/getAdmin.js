...
router.post("/", (req, res) => {
  ...
  const usersFile = path.join(__dirname, "../data/users.txt");  // Path to users file
  ...
  fs.readFile(groupsFile, "utf8", (err, data) => {
    ...
    let isSuperAdmin = false;  // Variable to store super admin status
    ...
    // Step 3: Read the users.txt file to check if the user has the superAdmin role
    fs.readFile(usersFile, "utf8", (err, userData) => {
      ...
      // Step 4: Check if the user has a superAdmin role
      const user = usersData.find(u => u.username === username);
      if (user && user.roles.includes("superAdmin")) {
        isSuperAdmin = true;  // Set to true if the user has superAdmin role
      }
      // Step 5: Read the channels.txt file to check banned users
      fs.readFile(channelsFile, "utf8", (err, channelData) => {
        ...
        // Step 6: Check each channel to see if the user is banned or a member
        ...
        // Step 7: Return the group information along with the allowed channels, members, admin status, and superAdmin status
        res.json({
          channels: allowedChannels,  // Only return the channels the user is not banned from and is a member of
          members,
          isAdmin,  // Admin status for the group
          isSuperAdmin  // Super admin status for the user
        });
      });
    });
  });
