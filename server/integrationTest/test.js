...
describe("POST /api/joinChannel", function () {
    let client;
    let db;
    let channelsCollection;
    ...
    before(async function () {
        ...
        // Clear the channels collection before starting tests
        await channelsCollection.deleteMany({});
    });
    ...
    it("should return an error if username or newChannel is missing", async function () {
        let res = await chai.request(app)
            .post("/api/joinChannel")
            .send({ newChannel: "testChannel" });
    ...
    it("should return an error if the channel is not found", async function () {
        const res = await chai.request(app)
            .post("/api/joinChannel")
            .send({ username: "user1", newChannel: "nonExistentChannel" });

        res.should.have.status(200);
        res.body.should.have.property("valid").eql(false);
    });

    it("should return an error if the user is banned from the channel", async function () {
        // Insert a channel with a banned user
        await channelsCollection.insertOne({
            name: "testChannel",
            members: ["user1"],
            banned: ["user2"]
        ...
        const res = await chai.request(app)
            .post("/api/joinChannel")
            .send({ username: "user2", newChannel: "testChannel" });

        res.should.have.status(200);
        res.body.should.have.property("valid").eql(false);
    });

    it("should successfully add a user to the channel if not banned and not already a member", async function () {
        // Insert a new channel without the user as a member
        await channelsCollection.insertOne({
            name: "testChannel3",
            members: ["user1"],
            banned: []
        ...
        const res = await chai.request(app)
            .post("/api/joinChannel")
            .send({ username: "user2", newChannel: "testChannel3" });

        res.should.have.status(200);
        res.body.should.have.property("valid").eql(true);

        // Verify that the user was added to the channel
        const channel = await channelsCollection.findOne({ name: "testChannel3" });
        channel.members.should.include("user2");
    });

    it("should not add a user to the channel if they are already a member", async function () {
        // Insert a channel with user2 as a member
        await channelsCollection.insertOne({
            name: "testChannel2",
            members: ["user1", "user2"],
            banned: []
        ...
        const res = await chai.request(app)
            .post("/api/joinChannel")
            .send({ username: "user2", newChannel: "testChannel2" });

        res.should.have.status(200);
        res.body.should.have.property("valid").eql(true);

        // Ensure no duplicate entry is added to the members list
        const channel = await channelsCollection.findOne({ name: "testChannel2" });
        channel.members.length.should.eql(2); // user1 and user2 should remain in the list
    });
